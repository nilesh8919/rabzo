    ...
    public function index(Request $request)
      {
            ...
            $data =DB::table('mt_delivery_boy')->orderby('id','desc')
				->get();
				
			foreach($data as $key=>$row)
			{
				 $data1 =DB::table('mt_delivery_boy_meta')->where('delivery_boy_id',$row->id)
				->get();
				foreach($data1 as $row1)
				{
					$keys= $row1->meta_key ;
					$data[$key]->$keys = $row1->meta_value;
				}
				
			}			
              return view('admin.delivery_boy',[
                'results'=> $data,
